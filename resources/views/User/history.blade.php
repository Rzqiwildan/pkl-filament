    ...
    <div class="container mx-auto mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4" style="width: 90%;">
        @foreach ($user_histories as $user_history)
            @foreach ($user_history->pelatihan->photos as $photo)
                <a
                    href="{{ $user_history->pelatihan->jenis == 'online'
                        ? route('online.show', $user_history->pelatihan->id)
                        : route('offline.show', $user_history->pelatihan->id) }}">
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center justify-between mt-4">
                                ...
                                <div class="text-sm">
                                    <span class="text-[#1E40AF] font-medium py-1 px-2 rounded-full"
                                        style="border: 1px solid #1E40AF">{{ $user_history->pelatihan->jenis }}</span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-sm text-gray-700 font-medium">Tingkat:
                                    {{ $user_history->pelatihan->kesulitan }}</span>
                            </div>
                            <!-- <div class="flex items-center space-x-2">
                            ...
                            <div class="mt-4">
                                ...
                                <p class="text-lg font-bold">Rp
                                    {{ number_format($user_history->pelatihan->harga, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                </a>
